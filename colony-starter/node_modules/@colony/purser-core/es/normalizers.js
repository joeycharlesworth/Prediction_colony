...
export var derivationPathNormalizer = function derivationPathNormalizer(derivationPath) {
  var deSerializedDerivationPath = derivationPath.split(PATH.DELIMITER);
  return deSerializedDerivationPath.map(function (pathSection, index) {
    switch (index) {
      ...
      case 0:
        {
          var deviationPathHeader = pathSection.split(SPLITTER);
           ...
           */
          deviationPathHeader[1]].join(SPLITTER);
        }
      ...
      case 3:
        {
          var deviationPathChageIndex = pathSection.split(SPLITTER);
          return deviationPathChageIndex.map(function (value) {
            return parseInt(value, 10);
          }).slice(0, 2).join(SPLITTER);
        }
...
export var multipleOfTwoHexValueNormalizer = function multipleOfTwoHexValueNormalizer(hexValue) {
...
export var addressNormalizer = stringPrefixNormalizer.bind(null, MATCH.ADDRESS);
/**
...
export var hexSequenceNormalizer = function hexSequenceNormalizer(hexString) {
  var prefix = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  return stringPrefixNormalizer(MATCH.HEX_STRING, hexString.toLowerCase(), prefix);
};
/**
...
export var recoveryParamNormalizer = function recoveryParamNormalizer(recoveryParam) {
